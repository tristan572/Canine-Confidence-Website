...
The investment in building these foundations properly pays dividends throughout your dog's entire life, creating not just better behaviour, but a truly harmonious partnership between human and canine companions.`,
  ...
  async createPackage(packageData) {
    ...
    const newPackage = { ...packageData, id };
...
var BUSINESS_EMAIL = "info@canineconfidence.com.au";
...
async function registerRoutes(app2) {
  ...
  app2.get("/api/packages", async (req, res) => {
    try {
      ...
      let packages2;
      if (category) {
        packages2 = await storage.getPackagesByCategory(category);
      } else {
        packages2 = await storage.getPackages();
      }
      res.json(packages2);
    } catch (error) {
  ...
  app2.post("/api/create-payment-intent", async (req, res) => {
    try {
      ...
      const paymentIntent = await stripe.paymentIntents.create({
        ...
        // Convert to cents
        currency: "aud",
        // Australian dollars for Brisbane business
        metadata: {
          sessionId,
          itemCount: cartItems2.length.toString()
        }
      });
      res.json({
